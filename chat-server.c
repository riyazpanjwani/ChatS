...
void *handle_client(void *arg){
	...
	sprintf(buff_out, "Welcome %s to the World of ChatS\r\n", cli->name);
		...
		if(buff_in[0] == '-'){
			...
			if(!strcmp(command, "-QUIT")){
				...
				if(param){
					...
					param = strtok(NULL, " ");
					if(param){
						sprintf(buff_out, "[Private][%s]", cli->name);
						while(param != NULL){
							strcat(buff_out, " ");
...
int main(int argc, char **argv){
	...
	printf("Server Serving Forver on localhost:%d\n",port);
	...
	while(1){
		...
		//http://stackoverflow.com/questions/17106626/how-to-decrease-cpu-usage-when-it-reaches-to-100-when-using-while1-loop
		/* Reduce CPU usage */
		sleep(1);
	}
}
